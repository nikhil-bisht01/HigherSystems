...
const ServiceCard = ({data}) => {
  ...
  return (
    <div style={{fontFamily:"Inter , sans-serif"}} className='group bg-white flex items-center flex-col shadow-md h-full pb-2 rounded-md cursor-pointer hover:scale-105  transition  delay-150 duration-700'>
      ...
      <button className='group-hover:bg-[#005AE6] group-hover:text-white transition duration-700 mt-10 border-2 font-bold text-[#005AE6]  border-[#005AE6] rounded-[20px] w-fit px-6 py-2' onClick={()=>{navigate("/our-services/"+data.url);}}>Read More</button>
    </div>
  )
}
